...
DB=Uni_Flu_refseq_clustered85.fa;
DBin="$DBdir"/"$DB";
...
while [ "$count1" -le "$countT" ]; do
	...
	sample=$(cat samples.txt | awk 'NR=='"$count1");
		...
		rm "$MAPPINGdir"/"$sample".ref.name.lst;
			...
			for i in _seg1 _seg2 _seg3 _seg4 _seg5 _seg6 _seg7 _seg8;do
			
			echo $i;
			
				FileIn="$MAPPINGdir"/"$sample".refstats.tab;
			
					cat "$FileIn" | grep -P "$i\\t" | sort -k9,9 -nr | head -n1 | cut -f1 >> "$MAPPINGdir"/"$sample".ref.name.lst;
...
#  I/O files
		...
		LOG="$LOGS"/"$SAMPLE".ref.mapping.log;
			...
			mapPacBio.sh -Xmx20g in="$FileIn" out="$FileOut" ref="$REFin" nodisk ambiguous=best qin=33 ow >> "$LOG" 2>&1 ; ## only for selecting the proper reference

	samtools view -bShu "$FileOut" | samtools sort -m 2G -@ 3 - -o "$FileOut2"; 
	samtools index "$FileOut2";
#     4
samtools consensus -@ 48 -a -q -f fasta --show-ins yes --show-del yes "$FileOut2"  -o "$CONSout";
